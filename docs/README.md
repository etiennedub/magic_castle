...
### 6.3 Tailscale Funnel

**Limitations**:
* **No support for Web Services** yet. This only exposes SSH access over a domain name. Recommended only when there is no public IP available.  
* SSH is exposed on port `10000` instead of `22` due to a Tailscale limitation.  
* It is not necessary to run Tailscale on the Terraform host, but it is recommended. This is because DNS propagation can take a long time, and the cluster cannot be fully deployed until the domain is reachable. Running Tailscale on the Terraform host resolves this issue.  

Use a [Tailscale Funnel](https://tailscale.com/kb/1223/funnel) to expose the `public`-tagged instance with a domain name generated by Tailscale.  
This allows deploying a MC cluster without requiring a public IP.  

---

**Steps:**

1. Create a tag named `tag:terraform` in [Tailscale Access Control](https://login.tailscale.com/admin/acls/visual/tags).  
2. Ensure that the tag has funnel access in the [ACL rules](https://tailscale.com/kb/1223/funnel#funnel-node-attribute).  
   The `nodeAttrs` section must contain:  

```json
{
  "target": ["tag:terraform"],
  "attr":   ["funnel"]
},
```

3. Generate an [OAuth client](https://tailscale.com/kb/1215/oauth-clients#setting-up-an-oauth-client) with the following permissions:  
   `auth_keys, devices:core:read, devices:posture_attributes, devices:posture_attributes:read, policy_file`  

4. Export `TAILSCALE_OAUTH_CLIENT_ID` and `TAILSCALE_OAUTH_CLIENT_SECRET`.  

5. Find your [Tailnet DNS name](https://login.tailscale.com/admin/dns), which is required by the DNS module.  

6. Use the configuration below to deploy using Tailscale Funnel:

```hcl`
```
# Support for other providers (GCP, AWS, Incus, ...)
module "openstack" {
  ...
  cluster_name     = module.dns.config.device_name
  domain           = module.dns.config.domain
  tailscale_config = module.dns.config
}

module "dns" {
  source         = "git::https://github.com/ComputeCanada/magic_castle.git//dns/tailscale"
  tailnet_domain = "<replace with your tailnet domain>"
}

# Hostname to access the cluster
output "hostname" {
  value = "${module.dns.config.device_name}.${module.dns.config.domain}"
}
```
